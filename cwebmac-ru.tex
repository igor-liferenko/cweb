...
\def\ATH{\X\kern-.5em:Определения препроцессора\X}
...
\def\note#1#2.{\Y\noindent{\hangindent2em%
    \baselineskip10pt\eightrm#1~#2.\par}}
...
\def\A{\note{Смотри также секцию}} % xref for doubly defined section name
\def\As{\note{Смотри также секции}} % xref for multiply defined section name
...
% \P is paragraph sign
\def\Q{\note{На этот код ссылается секция}} % xref for mention of a section
\def\Qs{\note{На этот код ссылаются секции}} % xref for mentions of a section
...
%    \def\$##1{\egroup_{\,\rm##1}\bgroup}% suffix to constant % versions < 3.67
    ...
    \let\~=\oct \let\^=\hex {#1}$}}
\def\U{\note{Этот код используется в секции}} % xref for use of a section
\def\Us{\note{Этот код используется в секциях}} % xref for uses of a section
...
\def\normaloutput#1#2#3{\ifodd\pageno\hoffset=\pageshift\fi
 \shipout\vbox{
  ...
  \global\advance\abspageno by1}

\gtitle={вывод \.{CWEB}} % this running head is reset by starred sections
\mark{\noexpand\nullsec0{\the\gtitle}}
...
\def\ch{\note{Эти секции были изменены с помощью файла изменений:}
  \let\*=\relax}
...
\def\inx{\par\vskip6pt plus 1fil % we are beginning the index
  ...
  \outer\def\I##1, ##2.{\par\hangindent2em
    \noindent##1:\kern1em##2.} % index entry
...
\def\fin{\par\vfill\eject % this is done when we are ending the index
  ...
  \def\grouptitle{ИМЕНА СЕКЦИЙ}
  \let\topsecno=\nullsec
  \message{Section names:}
  \output={\normaloutput\page\lheader\rheader}
  \setpage
  \def\note##1##2.{\quad{\eightrm##1~##2.}}
  \def\Q{\note{Упоминается в секции}} % crossref for mention of a section
  \def\Qs{\note{Упоминается в секциях}} % crossref for mentions of a section
...
\def\con{\par\vfill\eject % finish the section names
  % if we are on even page and there is even number of pages in contents, insert empty page
  \ifodd\pageno\else
    \advance\pageno by -\abspageno % number of first page
    \advance\pageno by -\contentspagenumber\relax % number of pages in contents
    \ifodd\pageno\else
      \advance\pageno by\contentspagenumber\relax % restore \count0 value for last
      \advance\pageno by\abspageno                % page (stored in DVI file)
      \titletrue\null\vfill\eject
    \fi
  \fi
  ...
  \def\grouptitle{СОДЕРЖАНИЕ}
  \message{Table of contents:}
  \topofcontents
  \line{\hfil Секция\hbox to5em{\hss Страница}}
  \let\ZZ=\contentsline
...
\def\datethis{\def\startsection{\leftline{\sc\today\ в \hours}\bigskip
...
\def\datecontentspage{% changed in version 3.66
  \def\botofcontents{\vfill
   ...
   \bigskip
   \leftline{\sc\today\ в \hours}}} % timestamps the contents page
