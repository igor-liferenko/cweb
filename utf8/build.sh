...
# remove cweav-sort.ch and cweav-op.ch from "tie -c cweav-merged.ch" in first
...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-sub.ch $DIR/cweav-utf8.ch $DIR/cweav-alfop.ch $DIR/cweav-sort.ch $DIR/cweav-op.ch $DIR/cweav-format.ch $DIR/cweav-file.ch $DIR/cweav-mac.ch > build-cweb.out # ATTENTION: cweav-file.ch must be before cweav-mac.ch
  then cat build-cweb.out; exit; fi
...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-sort.ch $DIR/cweav-format.ch > build-cweb.out
  then cat build-cweb.out; exit; fi
if ! ./ctangle cweave.w cweav-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c cweave.c || exit
