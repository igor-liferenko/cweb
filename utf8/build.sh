...
if ! tie -c comm-merged.ch common.w $DIR/comm-utf8.ch $DIR/comm-file.ch $DIR/comm-mac.ch $DIR/comm-exten.ch $DIR/comm-show.ch $DIR/comm-fclose.ch $DIR/comm-lim.ch >build-cweb.out
...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-utf8.ch $DIR/cweav-sort.ch $DIR/cweav-prod.ch $DIR/cweav-sub.ch $DIR/cweav-format.ch $DIR/cweav-type.ch $DIR/cweav-show.ch $DIR/cweav-fclose.ch $DIR/cweav-lim.ch $DIR/cweav-file.ch $DIR/cweav-mac.ch > build-cweb.out # ATTENTION: cweav-file.ch must be before cweav-mac.ch
  then cat build-cweb.out; exit; fi
...
if ! tie -c ctang-merged.ch ctangle.w $DIR/ctang-utf8.ch $DIR/ctang-show.ch $DIR/ctang-file.ch $DIR/ctang-pp.ch $DIR/ctang-lim.ch > build-cweb.out
  then cat build-cweb.out; exit; fi
...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-sort.ch $DIR/cweav-prod.ch $DIR/cweav-type.ch $DIR/cweav-show.ch > build-cweb.out
  then cat build-cweb.out; exit; fi
if ! ./ctangle cweave.w cweav-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c cweave.c || exit
clang -g -o cweave cweave.o common.o
