...
# remove cweav-sort.ch, cweav-nospace.ch and cweav-op.ch from "tie -c cweav-merged.ch" in first
...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-sub.ch $DIR/cweav-utf8.ch $DIR/cweav-alfop.ch $DIR/cweav-sort.ch $DIR/cweav-nospace.ch $DIR/cweav-op.ch $DIR/cweav-format.ch $DIR/cweav-file.ch $DIR/cweav-mac.ch > build-cweb.out # ATTENTION: cweav-file.ch must be before cweav-mac.ch
  then cat build-cweb.out; exit; fi
...
sed -i 's/@d xislower(c) (islower(c)&&((eight_bits)c<0200))/@d xislower(c) islower((eight_bits)c)/' common.h # if we do not use "(eight_bits)", islower('я') returns 0
...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-sort.ch $DIR/cweav-nospace.ch $DIR/cweav-format.ch > build-cweb.out
  then cat build-cweb.out; exit; fi
if ! ./ctangle cweave.w cweav-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c cweave.c || exit
...
clang -g -w -c common.c
clang -g -o ctangle ctangle.o common.o
if ! tie -c comm-merged.ch common.w $DIR/../comm-blank.ch $DIR/../comm-opts.ch $DIR/../comm-out.ch $DIR/../comm-pipe.ch > build-cweb.out
  then cat build-cweb.out; exit; fi
if ! ./ctangle common.w comm-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c -DCWEBINPUTS=\"/home/user/0000-git/cweb\" common.c || exit
if ! tie -c cweav-merged.ch cweave.w $DIR/../cweav-opts.ch $DIR/../cweav-tran.ch $DIR/../cweav-pipe.ch > build-cweb.out
  then cat build-cweb.out; exit; fi
if ! ./ctangle cweave.w cweav-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c cweave.c || exit
