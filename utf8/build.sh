...
if ! tie -c cweav-merged.ch cweave.w $DIR/cweav-sub.ch $DIR/cweav-utf8.ch $DIR/cweav-sort.ch $DIR/cweav-nospace.ch $DIR/cweav-format.ch $DIR/cweav-file.ch $DIR/cweav-mac.ch > build-cweb.out # ATTENTION: cweav-file.ch must be before cweav-mac.ch
  then cat build-cweb.out; exit; fi
...
if ! ./ctangle common.w comm-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c -DCWEBINPUTS=\"/home/user/0000-git/cweb\" common.c || exit
if ! tie -c cweav-merged.ch cweave.w $DIR/../cweav-opts.ch $DIR/../cweav-tran.ch > build-cweb.out # ATTENTION: cweav-out.ch must be before cweav-tran.ch
  then cat build-cweb.out; exit; fi
if ! ./ctangle cweave.w cweav-merged.ch > build-cweb.out; then cat build-cweb.out; exit; fi
clang -g -w -c cweave.c || exit
