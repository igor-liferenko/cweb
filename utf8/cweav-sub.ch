...
We use two pipes with identical content, because CWEAVE (i.e., cweave-null) reads each file
twice. Note, that the whole output is made to the pipe before the pipe is read.
This is OK because /dev/null-sections are small enough to fit into the pipe buffer.
