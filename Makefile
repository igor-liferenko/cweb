all:
	...
	ctie -c cweav-merged.ch cweave.w common-utf8.ch cweav-utf8.ch cweav-va.ch cweave+c.ch cweave+d.ch cweave+g.ch cweave+y.ch cweave+z.ch common-constants.ch cweav-constants.ch >/dev/null
	bin/ctangle cweave cweav-merged
	gcc -w -o cweave cweave.c common.c
	ctie -c ctang-merged.ch ctangle.w common-utf8.ch ctang-utf8.ch ctang-bhp.ch ctang-pre.ch ctangle+u.ch common-constants.ch ctang-constants.ch >/dev/null
	bin/ctangle ctangle ctang-merged
	gcc -w -o ctangle ctangle.c common.c
	@test -z "$$(sed -n 's/\(#define max_file_name_length [0-9]\+\).*/\1\\b/p' common.c | grep -L -f - cweave.c ctangle.c)"
