\input tugboat.sty
\openin0=tugboat.dates
\ifeof0 \let\writelastpage=\gobble
\else \closein0 \input tugboat.dates \TUBfinal\fi

%%% Start of metadata %%%
\title * \acro{UTF}-8 installations of \acro{CWEB} *
\author * Igor Liferenko *
\address * \tt igor.liferenko (at) gmail dot com *
%\address Ovrazhnaya 49-1
%         Zelenogorsk, Krasnoyarsk krai, 663691
%         RUSSIA
%%% End of metadata %%%
...
\acro{ASCII} characters to themselves, like this:
\verbatim
xchr[32] = ' ';
\endverbatim
\medskip
...
\endverbatim
\medskip
We do not use the {\it translit\/} array when outputting non-\acro{ASCII}
characters in C~identifiers.
So, in |ctangle.w| we again convert each such character
to the external encoding and then to \UTF-8 via {\it fprintf\/} using
the |%lc| conversion specifier.
\verbatim
< C_printf("%s",
