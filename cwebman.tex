...
\input cwebmac
\pdfhorigin=3.5cm \hoffset=\pdfpagewidth \advance\hoffset-\hsize \advance\hoffset-2\pdfhorigin
\def\page{\box255 } \normalbottom
...
one-letter identifiers (except when they are defined).
...
section. A tab mark or form feed character is equivalent to a space when it follows an \.@
...
of the time, but sometimes you will prefer to force a line break.
If a comment follows, say `\.{@/@,}' to break the line
before the comment. FIXME: never encountered such a use case...
...
you can replace one included file with another.
If \.{@i} \.{foo} is encountered in master file and the next match in change file
is not \.{@i} \.{foo}, further master file input is taken from \.{foo};
if the next match is \.{@i} \.{foo}, the substitution is done without giving
any treatment to \.{@i}.
After applying change-file, the output is checked for \.{@i} lines.
So, changes
can be made to lines from files that were included in master file; but if \.{@i} \.{foo}
occurs between \.{@y} and \.{@z} in a change file, individual lines
of file \.{foo} and files it includes are not changeable.
...
or \.{CWEAVE} won't print it correctly. FIXME: never encountered such a use case - what is this about?
...
\point 5. Verbatim listings are impossible (CWEAVE generates its own \TeX\ code,
so line breaks may not be retained, etc. - see \\{line\_length} parameter).

\point 6. The \.{\\title} macro will appear at the top of each page
in small caps; it is the job name unless redefined.

\point 7. The first page of table of contents usually is assigned page
number 1. To start on page 16 say `\.{\\def\\contentspagenumber{16}}'.
