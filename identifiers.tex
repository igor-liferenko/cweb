...
% sa.@t\0@>@=sa_handler@> = child_handler;
\def\0{\egroup\begingroup\def\vb##1{\endgroup\\{##1}}\bgroup}
% UPDATE: identifiers should be in index anyway, so use @:sa_handler@> right after @=sa_handler@>
% to put it in index and in cweave wrapper \9 is changed to underline the entry to distinguish it
% TODO: create one control command which simply uses code from these three control commands
